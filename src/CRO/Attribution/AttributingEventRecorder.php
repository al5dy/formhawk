...
final class AttributingEventRecorder implements EventRecorderInterface {
	...
	public function record_success( $provider, $provider_form_id, $title, $page_path, array $context = array() ) {
		...
		if ( $result ) {
			$this->record_provider_success( $provider, $provider_form_id );
			if ( $this->outcomes ) {
	...
	private function record_provider_success( $provider, $provider_form_id ) {
		if ( $this->valid_context( $provider, $provider_form_id ) ) {
			$this->contexts->consume_provider_success( $provider, $provider_form_id );
		}
	}
	...
	private function record( $provider, $provider_form_id, array $increments ) {
		$context = $this->valid_context( $provider, $provider_form_id );
		if ( ! $context ) {
			return;
		}
	...
	private function valid_context( $provider, $provider_form_id ) {
